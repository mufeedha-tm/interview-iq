...
| MongoDB Atlas Connected | In use (configured via `MONGODB_URI`; add Atlas proof screenshot/link in your final submission package) |
| GitHub Repo Link | Done (`https://github.com/mufeedha-tm/interview-iq`) |
| README with Screenshots | Pending manual submission step (`docs/screenshots/` still needs final images) |
| Demo Video (2-3 min) | Pending manual submission step (replace placeholder link below) |
...
## 10) Remaining Manual Submission Items

These items are not generated by the codebase and still need to be added by you before final submission:

- Add MongoDB Atlas proof:
  Include one screenshot or hosted link showing the Atlas cluster / connection in use.
- Add project screenshots to `docs/screenshots/`:
  Use the filenames `signup.png`, `dashboard.png`, `session.png`, and `results.png`.
- Replace the demo video placeholder:
  Update the link in the Demo Video section with your final 2-3 minute walkthrough URL.
...
Add your screenshots here after placing the image files in `docs/screenshots/`:
...
Replace this placeholder with your final demo video link:
